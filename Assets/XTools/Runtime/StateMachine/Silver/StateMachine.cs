...
namespace XTools.SM.Silver {
    public class StateMachine : SerializedMonoBehaviour {
        ...
        [NonSerialized] [OdinSerialize] State _root;
        ...
        public void StartSM() {
            ...
            Wire(_root, this, new HashSet<State>());
            _started = true;
            _root.Enter();
        }
        ...
        void Wire(State s, StateMachine m, HashSet<State> visited, State parent = null) {
            if (s == null) return;
        ...
        static bool IsStateList(Type type) {
            ...
            return typeof(State).IsAssignableFrom(elementType);
        }
    }
}